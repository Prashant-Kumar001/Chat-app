...
const removeMembers = asyncHandler(async (req, res) => {
    ...
    if (!chat.creator.equals(req.user._id)) {
        return ResponseHandler.error(res, 403, "You are not allowed to remove members from this chat");
    }
    ...
    emitEvent(req, ALERT, chat.members, `${user.username} has been removed from ${chat.name} group`);
    emitEvent(req, REFETCH_CHAT, chat.members);
...
const getChatDetails = asyncHandler(async (req, res) => {
    if (req.query.populate === "true") {
        ...
        return ResponseHandler.success(res, 200, "Chat found", chats);
    } else {
        const chats = await Chat.findById(req.params.id);
        if (!chats) {
            return ResponseHandler.error(res, 404, "Chat not found");
